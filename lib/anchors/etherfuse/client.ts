...
export class EtherfuseClient implements Anchor {
    ...
    async createCustomer(input: CreateCustomerInput): Promise<Customer> {
        ...
        try {
            // PLINA-MOD-001: SDF DevRel PIX_Guide.md exige `accountType: "business"`
            // no body de /ramp/onboarding-url — upstream original não envia. Sem isso,
            // sandbox aceita mas hosted onboarding pode falhar silenciosamente em
    ...
    async getQuote(input: GetQuoteInput): Promise<Quote> {
        ...
        // PLINA-MOD-002: SDF DevRel PIX_Guide.md documenta que `walletAddress`
        // deve ser passada no body do quote, não só no order. Upstream original
        // usa `stellarAddress` só pra `resolveAssetPair`. Sem `walletAddress`,
        // fees de onboarding pra wallet nova podem sair errados.
        const response = await this.request<EtherfuseQuoteResponse>('POST', '/ramp/quote', {
            quoteId,
            customerId: input.customerId || '',
            blockchain: this.blockchain,
            walletAddress: input.stellarAddress || '',
    ...
    async getKycUrl(
        ...
        // PLINA-MOD-001 (também em createCustomer): accountType obrigatório.
    ...
    /**
     * Simulate a crypto payment received event. **Sandbox only.**
     *
     * Off-ramp counterpart of {@link simulateFiatReceived} — required to
     * advance an off-ramp order in sandbox (SDF DevRel PIX_Guide.md §7.4).
     *
     * @param orderId - The order to simulate crypto receipt for.
     * @returns The HTTP status code from the Etherfuse API.
     */
    async simulateCryptoReceived(orderId: string): Promise<number> {
        // PLINA-MOD-003: helper para off-ramp sandbox que o upstream original
        // não expõe. Documentado no PIX_Guide.md §7.4.
        const url = `${this.config.baseUrl}/ramp/order/crypto_received`;
        console.log(`[Etherfuse] POST ${url}`, JSON.stringify({ orderId }));
    ...
    /**
     * Poll an on-ramp order until it reaches a terminal status or timeout.
     *
     * Etherfuse has a 3-10s indexing delay after order creation; immediate
     * polling returns 404. This helper retries 404s during a grace window
     * (`indexingGraceMs`) and only treats them as terminal failures after.
     *
     * PLINA-MOD-004: addition not present in upstream. Documented as a
     * critical gotcha by SDF DevRel (PIX_Guide.md §9 / Dev_Setup_Guide.md).
     */
    async pollOnRampUntilTerminal(
        orderId: string,
        opts: {
            intervalMs?: number;
            timeoutMs?: number;
        ...
        while (Date.now() < deadline) {
            const tx = await this.getOnRampTransaction(orderId);
            const notFound = tx === null;
            const status = tx?.status ?? null;
            opts.onTick?.({ status, notFound });

            if (notFound) {
                // 404 dentro da janela de indexing = aguardar; depois é falha.
                if (Date.now() - startedAt > indexingGraceMs) {
                    throw new AnchorError(
                        `Order ${orderId} not found after indexing grace (${indexingGraceMs}ms)`,
                        'ORDER_NOT_FOUND',
                        404,
                    );
                }
